...
- 自动 Gazebo 验收默认使用独立 `ROS_DOMAIN_ID=83`；Python 验收脚本是 domain 单一真源，支持 `FCR_TRON_ACCEPTANCE_ROS_DOMAIN_ID` 或 `--ros-domain-id` 覆盖，且拒绝空值、`0`、前导零、非十进制或越界值，避免和真实 TRON1 bringup 共用 ROS graph。
