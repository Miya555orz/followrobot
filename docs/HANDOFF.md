...
- `[UPDATED]` `tools/tron1_bringup/tron1_real_motion_path_preflight.sh` now returns nonzero `BLOCK` when `TRON_IP` is routed through proxy/TUN/container/policy-table paths, a non-wired-looking device, a device other than optional `TRON_LINK_IFACE`, or does not answer read-only ping. Exit codes are documented as `0=PASS, 1=FAIL, 2=WARN-only, 3=BLOCK`. The official `robot_hw` default `/cmd_vel` notice is INFO only because the FCR launch override must provide `/fcr_tron/cmd_vel`. This does not publish velocity or activate the controller.
